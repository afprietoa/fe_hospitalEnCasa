  ...
  export default {
    ...
    methods: {
        getData: async function () {
            ...
            axios.all([axios.get(`http://127.0.0.1:8000/user/${this.$route.params.id}/`),axios.get('http://127.0.0.1:8000/patient/')])
                     .then(axios.spread((...responses)=>{
                        let user = responses[0].data
                        let patients = responses[1].data
                        let hospitalized = []
                        console.log(user)
                        console.log(hospitalized)
                            patients.forEach(patient=>{
                                if(user.id == patient.user && user.id == this.$route.params.id){
                    ...
                    setTimeout(() => {
                        ...
                        icon: 'error',
                        title: 'ERROR !',
                        text: "Wrong in obtaining the patient's data"
                        })
                    }, 250)
                    // alert("Error")
        ...
        editRecord: async function () {
            ...
            let patient = {
                ...
                "birth_date": this.usr.birth_date, 
                "address": this.usr.address, 
                "city": this.usr.city, 
                "longitude": this.usr.longitude,
                "latitude": this.usr.latitude 
            }

            axios.all([
                axios.put(`http://127.0.0.1:8000/user/${this.$route.params.id}/`, user),
                axios.put(`http://127.0.0.1:8000/patient/${this.usr.id}/`, patient)
            ])
